...
`./gradlew publishPlugin` uploads the built zip to JetBrains Marketplace. It takes the Marketplace token
from the `PUBLISH_TOKEN` environment variable, or from an untracked `local.properties` next to
`build.gradle.kts`:
...
Generate the token at <https://plugins.jetbrains.com/author/me/tokens> (it is shown only once).
